const methods = {
    ...
    in: function (value, allowedArray) {
        if (allowedArray.indexOf(value) === -1)
            return false
        return true;
    },
...
const messages = {
    ...
    rangelength: "Please enter a value between {__low__val} and {__higher__val} characters long.",
    ...
    for (field in rules) {
        ...
        for (field__rule__key in field__rules) {
            ...
            if (field__rule__key == 'range' || field__rule__key == 'rangelength') {
                ...
                result = methods[field__rule__key].call(null, field__value, __lower__val, __higher__val);
                if (!result) {
                    let message = messages[field__rule__key].replace('{__field}', l.startCase(field)).replace('{__low__val}', __lower__val).replace('{__higher__val}', __higher__val)
                    field_errors.push(message)
                }
            } else if (field__rule__key === 'in') {

                result = methods[field__rule__key].call(null, field__value, field__rule__params);
                if (!result) {
                    let message = messages[field__rule__key].replace('{__field}', l.startCase(field)).replace('{__allowedValues__}', field__rule__params.toString())
                    field_errors.push(message)
                }
            } else {
                result = methods[field__rule__key].call(null, field__value, field__rule__params);
                if (!result) {
                    let message = messages[field__rule__key].replace('{field}', l.startCase(field)).replace('{__low__val}', field__rule__params)
                    field_errors.push(message)
                }
            }
        }
